...
#         state = self.state_task_mode # Change this to the state you want to test
# Currently the above line is commented out and if you want to use it then you'll have to remove the "#" in front.
# Removing that will disable the actual state the robot is in and set it to a specific state so that you can test that specific state.
        if state == self.state_active_idle: # If the state is active_idle then do the following code
        ...
        if state == self.state_active_detected:
        ...
        if state == self.state_standby_idle:
        ...
        if state == self.state_standby_detected:
            sequence1 = [(0, 0.1, 1, "QuadEaseIn")]
            loops1 = 0
